...
- 현재 구현은 `TaskStore`, `PlanStore`, `StepStore`를 in-memory projection으로 붙여 반환값과 동일한 최신 상태를 저장한다.
- 아직 없는 것:
...
| 의존성 | 역할 |
| --- | --- |
| `Task Store` | Task 최신 snapshot 저장/조회 |
| `Plan Store` | Plan 최신 snapshot 저장/조회 |
| `Step Store` | Step 최신 status, approval_id, plan별 목록 조회 |
...
- Store는 append-only event를 대체하지 않는다. Orchestrator가 계산한 최신 상태 snapshot만 저장한다.
- 첫 구현은 모두 in-memory store로 시작하고, 나중에 SQLite 등 영속 저장소로 교체 가능한 인터페이스를 유지한다.
...
- 계산된 최신 `Task`, `Plan`, `Step` snapshot을 각 Store에 write-through로 반영한다.
...
8. `policy.evaluated` 결과가 `require_approval` 또는 `deny`면 `risk.flagged`를 추가로 기록한다.
9. Step 결과를 계산한 뒤 `StepStore`, `PlanStore`, `TaskStore`에 같은 snapshot을 순서대로 반영한다.
10. 성공 시 다음 Step으로 진행한다.
11. 실패 시 현재 구현은 retry 없이 다음 Step으로 진행하거나 배치를 종료하지 않고, 각 Step 결과를 수집해 반환한다.
12. 모든 변화는 이벤트와 감사 로그에 남긴다.
...
4. `TaskStore.save(task)`와 `PlanStore.save(plan)`을 호출한다.
5. 각 Step을 `StepStore.save(step)`로 초기 `ready` snapshot으로 저장한다.
6. 각 Step마다 `action.started`를 발행한다.
7. Gateway 실행 결과에 따라 아래 셋 중 하나를 발행한다.
   - `action.succeeded`
   - `step.approval_requested`
   - `action.failed`
8. `applyStepResults(...)`, `withUpdatedPlan(...)`, `withUpdatedTask(...)` 결과를 Store에 다시 쓴다.
9. 모든 발행 이벤트는 즉시 AuditLog에도 기록된다.
10. 반환값에는 `task`, `plan`, `approvals`, `steps`, `events`, `auditRecords`가 모두 포함된다.
...
2. `task.created`
3. `plan.drafted`
4. `TaskStore.save`, `PlanStore.save`, 각 Step에 대한 `StepStore.save`
5. 실행 대상으로 선택된 Step에 대해 `step.ready`
6. 같은 Step에 대해 `policy.evaluated`
7. `decision=allow`이므로 `risk.flagged`는 발행하지 않는다
8. `action.started`
9. Tool 실행 후 `action.succeeded`
10. 해당 Step을 포함한 최신 Step snapshot들을 `StepStore.save`
11. Step 결과를 반영한 `PlanStore.save` + `plan.updated`
12. 같은 결과를 반영한 `TaskStore.save` + `task.updated`

#### require_approval 경로

1. `task.created`
2. `plan.drafted`
3. `TaskStore.save`, `PlanStore.save`, 각 Step에 대한 `StepStore.save`
4. `step.ready`
5. `policy.evaluated` with `decision=require_approval`
6. `risk.flagged`
7. pending approval이 있으면 재사용하고 없으면 새 approval 생성
8. `step.approval_requested`
9. `waiting_approval + approval_id`가 반영된 Step snapshot을 `StepStore.save`
10. Step 상태 `waiting_approval`를 반영한 `PlanStore.save` + `plan.updated`
11. Task 상태 `waiting_approval`를 반영한 `TaskStore.save` + `task.updated`
...
4. `step.ready`
5. `policy.evaluated`
6. `decision=deny`이면 `risk.flagged`
7. `decision=deny` 또는 실행 실패 결과에 대해 `action.failed`
8. `failed | blocked`가 반영된 Step snapshot을 `StepStore.save`
9. 실패 결과를 반영한 `PlanStore.save` + `plan.updated`
10. 실패 결과를 반영한 `TaskStore.save` + `task.updated`
...
5. `step.approved`
6. 같은 Step을 `StepStore.save(ready)`로 되돌린다
7. 같은 Step에 대해 다시 `step.ready`
8. `approval_granted=true` 컨텍스트로 `policy.evaluated`
9. `decision=allow`이면 `risk.flagged`는 발행하지 않는다
10. `action.started`
11. 결과로 `action.succeeded` 또는 `action.failed`
12. 결과에 맞는 Step snapshot으로 `StepStore.save(...)`
13. Step 결과를 반영한 `PlanStore.save` + `plan.updated`
14. Task 결과를 반영한 `TaskStore.save` + `task.updated`

#### denied | expired

1. `approval_id`를 resolve한다
2. `step.denied`
3. 같은 Step을 `StepStore.save(blocked)` 또는 최소 `failed` 규칙으로 갱신한다
4. `PlanStore.save` + `plan.updated`
5. `TaskStore.save` + `task.updated`
6. Tool 실행은 일어나지 않는다

### 상태 전이 원칙

- 현재 구현은 상태 필드보다 이벤트 스트림이 진실원본에 가깝다.
- Store 도입 후에도 이벤트 스트림은 진실원본이고, Store는 최신 조회용 projection이다.
- 한 번의 실행 시도는 하나의 `action_id`와 연결된다.
- 승인 후 재개는 기존 Step을 교체하지 않고 같은 Step 정의를 다시 실행한다.
- `resumeApproval(...)`는 현재 `resolveApproval(...)`의 별칭이다.

## 5. Store Write Contract

최소 in-memory store 구현은 아래 호출 순서를 지켜야 한다.

### `run(...)`

1. `task.created` 직후 `TaskStore.save`
2. `plan.drafted` 직후 `PlanStore.save`
3. `plan.steps` 생성 직후 각 Step에 대해 `StepStore.save`
4. 각 Step 처리 직후 최신 Step snapshot을 `StepStore.save`
5. 최종 Plan 계산 직후 `PlanStore.save`
6. 최종 Task 계산 직후 `TaskStore.save`

### `resumeApproval(...)`

1. approval resolve 성공 후 Step 존재 확인
2. approved면 재실행 직전 `StepStore.save(ready)`
3. 재실행 결과 직후 최신 Step snapshot을 `StepStore.save`
4. 결과 반영 후 `PlanStore.save`
5. 결과 반영 후 `TaskStore.save`
6. denied/expired면 Tool 실행 없이 `StepStore -> PlanStore -> TaskStore` 순서로 종료 상태 기록

Store 실패 처리:

- 첫 in-memory 단계에서는 store write 실패를 별도 recoverable path로 두지 않는다.
- store write가 실패하면 Orchestrator 전체 호출을 실패로 본다.
- 단, 이미 발행된 이벤트/AuditLog는 되돌리지 않는다. 이 불일치는 다음 영속 저장소 단계에서 reconciliation 대상으로 남긴다.
...
- 현재 구현은 `policy.evaluated`가 `require_approval` 또는 `deny`일 때만 `risk.flagged`를 추가 발행한다.
- lifecycle 이벤트 구현 후에는 한 Step 시도에서 `step.ready -> policy.evaluated -> risk.flagged? -> action.* | step.approval_requested -> plan.updated -> task.updated` 순서를 유지해야 한다.
