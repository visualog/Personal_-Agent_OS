...
- 아직 없는 것:
  ...
  - `task.updated`, `plan.updated`, `step.ready`, `policy.evaluated`의 실발행
...
| 이벤트 | 발행 시점 |
| --- | --- |
| `step.ready` | 정의만 존재, 현재 런타임은 아직 발행하지 않음 |
| `policy.evaluated` | 정의만 존재, 현재 런타임은 아직 발행하지 않음 |
...
| `task.updated` | 정의만 존재, 현재 런타임은 아직 발행하지 않음 |
