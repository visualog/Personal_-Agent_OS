...
4. `deny`면 Step을 실행하지 않고 실패 또는 차단 상태로 기록한다.
5. `require_approval`이면 Approval Flow에 승인 요청을 만들고 `waiting_approval` 상태로 둔다.
6. `allow`이면 Tool Gateway로 실행 요청을 보낸다.
7. 실행이 시작되면 Step 상태를 `running`으로 바꾼다.
8. Tool 결과를 수신하면 Step 상태를 `succeeded` 또는 `failed`로 바꾼다.
9. 성공 시 다음 Step으로 진행한다.
10. 실패 시 retry 정책 또는 중단 정책을 적용한다.
11. 모든 변화는 이벤트와 감사 로그에 남긴다.

### 상태 전이 원칙

- Step은 `pending -> ready -> running -> succeeded | failed | skipped` 흐름을 따른다.
- 승인 대기 Step은 `ready -> waiting_approval -> ready | denied` 흐름을 따른다.
- 재시도는 기존 Step을 덮어쓰지 않고 별도 실행 시도로 기록한다.
- 한 번의 실행 시도는 하나의 `action_id`와 연결되어야 한다.
...
| 이벤트 | 발행 시점 |
| --- | --- |
| `step.ready` | Step이 실행 가능 상태가 되었을 때 |
| `policy.evaluated` | Step 실행 전 판정이 끝났을 때 |
...
| `task.updated` | 전체 Task 상태가 변경되었을 때 |
...
| --- | --- |
| `policy_denied` | 실행 중단, 감사 기록, Step 실패 또는 보류 |
| `approval_missing` | `waiting_approval` 유지 |
| `approval_denied` | Step 종료, 다음 Step 진행 여부 재판정 |
| `tool_timeout` | timeout 결과로 표준화 후 retry 판단 |
| `tool_crash` | retry 가능 여부에 따라 재시도 또는 실패 |
| `transient_io` | 지정된 횟수까지 재시도 가능 |
| `result_invalid` | 실패 처리 후 감사 기록 |
| `plan_inconsistent` | Plan 재조회 또는 재계획 필요 |

### Retry 규칙

- retry는 retryable 실패에만 허용한다.
- retry 횟수와 backoff는 Step 또는 정책 기준을 따른다.
- 동일 실패가 반복되면 fail-fast 또는 replan으로 전환한다.
- retry 시에도 새 감사 레코드를 남긴다.
...
- `ready` Step이 `allow` 판정을 받으면 `action.started`와 `action.succeeded`가 순서대로 발생해야 한다.
- 성공한 Step 이후 다음 Step이 자동으로 `ready` 또는 `running`으로 넘어가야 한다.

### 승인 필요

- `require_approval` Step은 승인 없이 실행되면 안 된다.
- 승인 요청 이벤트가 남고 Step 상태가 `waiting_approval`이어야 한다.
...
- Step은 실행되지 않은 채 종료 또는 보류되어야 한다.
...
- `policy.evaluated` 없이 `action.started`가 발생하면 안 된다.
